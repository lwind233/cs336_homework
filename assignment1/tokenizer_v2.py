                    ...
                    if result is False:
                        ...
                        if tmp_len != 2:
                            ...
                            if result == 0:
                                ...
                                t1 = e_f+e_b
                                t2 = e_0+t1
                                tmp_frequncy[t1] = tmp_frequncy[t1] - value
                                if tmp_frequncy[t1] ==0 :
                                    tmp_frequncy.pop(t1)
                                tmp_frequncy[t2] = tmp_frequncy.get(t2,0) + value
                            ...
                            # 需要合并的两个token在中间，且长度大于等于4,则合并token后，前后跟着的那个token对的出现数量要降低，新出现的token数量增加
                                ...
                                if tmp_frequncy[t1] ==0:
                                    tmp_frequncy.pop(t1)
                                tmp_frequncy[t2] = tmp_frequncy[t2] - value
                                if tmp_frequncy[t2] ==0:
                                    tmp_frequncy.pop(t2)
                                tmp_frequncy[t3] = tmp_frequncy.get(t3,0) + value
                                tmp_frequncy[t4] = tmp_frequncy.get(t4,0) + value
